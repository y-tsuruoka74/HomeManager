...
let
  ...
  copilotConfigSync = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      jq -s --argjson entries "$HERDR_SESSION_START_JSON" \
        -f ${../../scripts/merge-copilot-hooks.jq} \
...
in
{
  ...
  home.activation.copilotConfigSync = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    HERDR_SESSION_START_JSON=${lib.escapeShellArg (builtins.toJSON herdrSessionStart)} ${copilotConfigSync}/bin/copilot-config-sync
  '';
